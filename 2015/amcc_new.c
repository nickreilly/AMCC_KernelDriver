 ...
 *----------------------------------------------------------------------------*/
struct amcc_priv gb_dev;           /* Private device data */
...
static short *phy_mem = NULL; // pointer to dma-able memory block
dma_addr_t bus_addr = 0; // pointer to dma-able memory block
static short *phy_buf = NULL; // virtual pointer to physical memory block
...
long ociw_physbuf = 0; // location of physical buffer
long ociw_physbufsize = 0; // size of memory we want to use

__MODULE_PARM_TYPE(ociw_physbuf,"l"); // location of physical buffer start
...
amcc_probe (struct pci_dev* pcidev, const struct pci_device_id* amcc_device_id)
{
    ...
    rc = register_chrdev(AMCC_MAJOR, AMCC_NAME, &amcc_fops);
    if (rc == 0)
    ...
    if ((p2a.buf == NULL) || (a2p.buf == NULL)) {
        ...
        unregister_chrdev (gb_dev.major, AMCC_NAME);
        return (-ENOMEM);
    }
    ...
    if (ociw_physbufsize) // physical buffer available?
    {
	    ...
	    phy_mem = kmalloc(32768, GFP_KERNEL); // alloc for dma
		if (ociw_physbuf)
			phy_buf = ioremap(ociw_physbuf, ociw_physbufsize); // alloc for pixels.
		else
			phy_buf = vmalloc(ociw_physbufsize);
		//	phy_mem = pci_alloc_consistent(pcidev, 16384, &bus_addr);
		// bus_addr = pci_map_single(gb_dev, phy_mem, 8192, PCI_DMA_FROMDEVICE);
    ...
    amcc_write_opreg ((u32) 0, AMCC_OP_REG_ICSR);
    amcc_write_opreg ((u32) 0x00001800, AMCC_OP_REG_PTCR);
...
amcc_remove (struct pci_dev* pcidev)
{
    ...
    {
        kfree(phy_mem);
		//pci_free_consistent(pcidev, 16384, phy_mem, bus_addr);
    }
...
int dest __attribute__ ((aligned (16))) = 0;
int dest2 __attribute__ ((aligned (16))) = 0;
int dest3 __attribute__ ((aligned (16))) = 0;
int dest4 __attribute__ ((aligned (16))) = 0;
int dest5 __attribute__ ((aligned (16))) = 0;
int dest6 __attribute__ ((aligned (16))) = 0;
int dest7 __attribute__ ((aligned (16))) = 0;
int dest8 __attribute__ ((aligned (16))) = 0;

static int
amcc_ioctl (struct inode *inode, struct file *file,
    unsigned int cmd, unsigned long arg)
{
    ...
    case AMCC_PUT_RCR:          /* write RCR register */
		{
		u32  rcr;
		if(get_user (rcr, (u32 *) arg) == -EFAULT)
			return -EFAULT;
		amcc_write_opreg (rcr, AMCC_OP_REG_RCR);
        break;
		}
    case AMCC_GET_PTCR:         /* read PTCR register */
		...
		src = (u64*) gb_dev.pt1_virt_addr;
		src = (u64*) ((int)src & 0xFFFFFFF0); // no crash
		asm("movaps %1, %%xmm0;\n\
			 movaps %%xmm0, %0;"
				// :"=m"(*((u64*)(((int)(dest))&0xFFFFFFF0))) // no crash.
				:"=m"(dest)
				:"m"(*src)
				:"%xmm0"
				);
...
static inline void nextTail(void)
{
    pixqueue_tail++; // move ahead two bytes. (== one short)
    if (pixqueue_tail >= phy_buf_end){
        pixqueue_tail = phy_buf;
    }
}
...
int channel = 3;
	...
	set_dma_count(channel, 8192);
	enable_dma(channel);
	while (get_dma_residue(3))
	{
...
#define save_cr0_and_clts(x) __asm__ __volatile__ ("movl %%cr0,%0;\n\
													 clts" :"=r" (x))
#define save_fpenv(x)        __asm__ __volatile__ ("fxsave %0" : "=m" (x))
#define restore_fpenv(x)     __asm__ __volatile__ ("fxrstor %0" : "=m" (x));
#define restore_cr0(x)       __asm__ __volatile__ ("movl %0,%%cr0": :"r" (x));

unsigned long cr0;
unsigned long linux_fpe[128]__attribute__((aligned(512)));
...
save_fpenv(linux_fpe);   
	...
	blockxfer :
	// can't enter if pixqueue_tail not on longlong boundary
	while ((!(((int)pixqueue_tail) & 0xF)) && !((r = amcc_read_opreg(AMCC_OP_REG_IMB)) & IMB_RX_HF))
	{
		int i = 0;
		int max = phy_buf_end - pixqueue_tail;
		if (max >= 4096) 
			max = 4096;
		max &= 0x00FFFC;
		//
		// approaching 2M pix per second.

//		memcpy((u32 *)phy_mem, (u32 *)gb_dev.pt2_virt_addr,  max*2);
//		memcpy((u32 *)pixqueue_tail, (u32 *)phy_mem, max*2);
		// memcpy((u32 *)pixqueue_tail, (u32 *)gb_dev.pt2_virt_addr,max*2);

		//dest = (u64*) pixqueue_tail;
		// GCC uses AT&T syntax, NOT Intel syntax!!!
		// mov src, dest == AT&T syntax!!
		if (0)
		{
		// 128 bit transfers
		u64 *src;
		src = (u64*) gb_dev.pt1_virt_addr;
		src = (u64*) ((int)src & 0xFFFFFFF0); // no crash
		for (i=0; i<max; i+=8) // read 8 pix at a time. 
		{
		asm("movaps %1, %%xmm0;\n\
			 movaps %%xmm0, %0;"
				// :"=m"(*((u64*)(((int)(dest))&0xFFFFFFF0))) // no crash.
			...
			pixqueue_tail[i] = ((u16*)&dest)[0];
			pixqueue_tail[i+1] = ((u16*)&dest)[1];
			pixqueue_tail[i+2] = ((u16*)&dest)[2];
			pixqueue_tail[i+3] = ((u16*)&dest)[3];
			pixqueue_tail[i+4] = ((u16*)&dest)[4];
			pixqueue_tail[i+5] = ((u16*)&dest)[5];
			pixqueue_tail[i+6] = ((u16*)&dest)[6];
			pixqueue_tail[i+7] = ((u16*)&dest)[7];
			pixqueue_tail[i] = i;
		}
		}
		else if(0) {
		u64 *src;
		src = (u64*) gb_dev.pt2_virt_addr;
		src = (u64*) ((int)src & 0xFFFFFFF0); // no crash
		for (i=0; i<max; i+=8) // read 8 pix at a time. 
		{
		asm("movq %2, %%mm0;\n\
			...
			pixqueue_tail[i+2] = ((u16*)&dest)[2];
			pixqueue_tail[i+3] = ((u16*)&dest)[3];
			pixqueue_tail[i+4] = ((u16*)&dest5)[0];
			pixqueue_tail[i+5] = ((u16*)&dest5)[1];
			pixqueue_tail[i+6] = ((u16*)&dest5)[2];
			pixqueue_tail[i+7] = ((u16*)&dest5)[3];
			pixqueue_tail[i] = i;
		}
		}
		else if(0) {
		...
		u32 dest4;
		u32 *src;
		src = (u32*) gb_dev.pt2_virt_addr;
		for (i=0; i<(max>>1); i+=4) // read 8 pix at a time. NOT FASTER.
		{
		asm("movd %4, %%mm0;\n\
			 movd %5, %%mm1;\n\
			 movd %6, %%mm2;\n\
			 ...
			 movd %%mm2, %2;\n\
			 movd %%mm3, %3;"
				:"=m"(dest), "=m"(dest2), "=m"(dest3), "=m"(dest4)
				:"m"(*src), "m"(*(src+1)), "m"(*(src+2)), "m"(*(src+3))
				:"%mm0", "%mm1", "%mm2", "%mm3"
				);
			((u32*)pixqueue_tail)[i] = dest;
			((u32*)pixqueue_tail)[i+1] = dest2;
			((u32*)pixqueue_tail)[i+2] = dest3;
			((u32*)pixqueue_tail)[i+3] = dest4;
			src+=4;
		}
		}
		else {
			_mmx_memcpy((u32 *)pixqueue_tail, (u32 *)gb_dev.pt2_virt_addr,max*2); // this works too.
		}
		// dma_prepare();
		//for (i=0; i<(max>>1); i++)
			 //((long *)pixqueue_tail)[i] = readl(((u32*)(gb_dev.pt2_virt_addr))+i);
		//for (i=0; i<max; i++)
			//((short *)pixqueue_tail)[i] = readl(((u32*)(gb_dev.pt1_virt_addr)));
		pixqueue_tail += (max-1);
		nextTail();
		if (max < 4096) 
		{
			_mmx_memcpy((u32 *)pixqueue_tail, (u32 *)gb_dev.pt2_virt_addr,(4096-max)*2); // this works too.
			pixqueue_tail += 4096 - max;
		}
	}
	xfers = 0;
    while((r = amcc_read_opreg(AMCC_OP_REG_IMB)) & IMB_RX_RDY)
    {
		// read the next pixel
		if ((!(((int)pixqueue_tail) & 0xF)) && (!(r&IMB_RX_HF))) // on long boundary and half full
			goto blockxfer ; // switch to block xfer mode.
        if (xfers>8192)
...
static irqreturn_t ociw_irq_handler (int irq, void *dev)
{
	u32 icsr = 0;
...
static struct pci_driver amcc_driver = {
    ...
    .id_table = id_table,
    .probe = amcc_probe,
    .remove = __devexit_p(amcc_remove)
};
...
amcc_init (void)
{
    ...
    printk (KERN_INFO "amcc_init: AMCC S5920 Rev 2.0a for 2.6 kernel \n");
