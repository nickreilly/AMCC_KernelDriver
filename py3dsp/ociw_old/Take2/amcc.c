 ...
 * 
 * v3.0-    Updating for use on the kernel 5.15
 *           -Needs some pretty heavy verification to make sure it works
 *           -But it is compiling and seems to be doing the right thing?
 *----------------------------------------------------------------------------*/
#ifndef __KERNEL__
# define __KERNEL__
#endif
...
module_param(ociw_physbufsize, long, 0644); // size of physical buffer in bytes
...
amcc_probe (struct pci_dev* pcidev, const struct pci_device_id* amcc_device_id)
{
    ...
    if ( request_irq(gb_dev.irq, 
                 &ociw_irq_handler,IRQF_SHARED | IRQF_NO_THREAD,"ociw",(void*)&gb_dev)<0)
    ...
    while (a2p.bytes_done < a2p.copy_size && retrys < AMCC_MAX_RETRYS) {
        ...
        if (r & IMB_RX_RDY) {
            if (a2p.copy_size - a2p.bytes_done > 255 && (r & IMB_RX_PAE)) {
                for (k = 0; k < 127; k++) {
                    rbuf[j] = (u16)readl((u32 *)(gb_dev.pt1_virt_addr + k * 2));
                    j++;
                    a2p.bytes_done += 2;
                }
            }
            else {
                rbuf[j] = (u16)readl((u32 *)(gb_dev.pt1_virt_addr));
                a2p.bytes_done += 2;
                j++;
            }
            retrys = 0;
...
amcc_init (void)
{
    ...
    printk (KERN_INFO "amcc_init: AMCC S5920 Rev 3.0 for 5.15 kernel \n");
