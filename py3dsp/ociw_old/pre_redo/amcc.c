...
// Version 3.0 (for python3)
// 2023 Feb 
// Still trying to get compatibility on 64 bit working!
...
amcc_probe (struct pci_dev* pcidev, const struct pci_device_id* amcc_device_id)
{
    ...
    printk (KERN_DEBUG "amcc_probe: PT[%d] Size %8d  Phys %08x  Virt %p\n",
    ...
    if ( request_irq(gb_dev.irq, 
            &ociw_irq_handler,IRQF_SHARED | 0,"ociw",(void*)&gb_dev)<0)
...
amcc_read(struct file *file, char *buf, size_t count, loff_t *ppos)
{
    ...
    a2p.copy_size = min (sizeof count, sizeof a2p.size);
...
amcc_write(struct file *file, const char *buf, size_t count, loff_t *ppos)
{
    ...
    while (count > 0){
        p2a.bytes_done = 0;
        p2a.copy_size = min (sizeof count, sizeof p2a.size);
