...
logger = log.getLogger("pydsp.ociw")
...
class PCIDevice:
    ...
    def open(self, devname="/dev/amcc0"):# "/etc/udev/devices/ociw0"):
        "Open the device. (perhaps a bad name, Python has a built-in named open.)"
        # if not self.dev:
        try:
            self.dev = open(devname, mode="rb+", buffering=0)
...
class RawFrame:
    ...
    def __init__(self, nrows, ncols, prepix=0, postpix=0, dev=dev):
        """
