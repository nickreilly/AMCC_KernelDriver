...
print "Which filter? (3.3, l', or cvfII)"
filterchoice = str(raw_input())

if 'cvf' not in filterchoice:
    filterBase.set(str(filterchoice))
    
else:
    print "Which wavelength?"
    wave_temp = int(raw_input())
    filterBase.set((filterchoice, wave_temp))
...
print 'How many of these ramps do you want?'
...
for i_ramp in range(numramps):
  for detbias in range(startbias, endbias+1, stepbias):
      ...
      #rd.object = "linearity%dK_%dmV_arrayreset"%(CurrTemp, detbias)
