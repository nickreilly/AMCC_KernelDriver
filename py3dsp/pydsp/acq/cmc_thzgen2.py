...
"""
# 
bestVgs295K = numarray.array([[477, 476, 450, 478, 473, 433, 448], 
...
bestVgs259K = numarray.array([[483, 477, 453, 478, 472, 431, 453], 
...
bestVGs259K2 = numarray.array([[494, 491, 478, 490, 484, 450, 465],
...
bestVGs259K3 = numarray.array([[483, 481, 460, 481, 480, 439, 457],
...
def gen2on():
    ...
    dd.vgs      =480
    dd.biascurr =900
    dd.vcm      =0 # 1650 for CDS common mode 
    dd.vdd_cds  =0 # 3300 for CDS 
    dd.vload    =0
    dd._vreset  =3300
    dd.dsub     =3300
    dd.bias14   =0
    ...
    dd.clk_RSlo =0
    dd.clk_RShi =3300
    # Global Reset = 0 gives full reset of pixels
    dd.glbRstlo =0
    # When Global Reset >0, then we have current source, 
    # until Global Reset = 3.3V, which turns it all off -- photovoltaic mode. 
    # Zeljko says Global Reset should be 2.60V to get ~0.1uA like Greg used.
    dd.glbRsthi =2600  
...
def varyVgs(VgsStart,VgsEnd,VgsStep,NumImg):
    ...
    OrigVgs = dd.vgs
    OrigVreset = dd.vreset
    for voltage in range(VgsStart,VgsEnd,VgsStep):
        ...
        outputfilename = xdir.get_nextobjfilename() + ".fits" # Get filename
...
def arraynoise(NumImgSamp, lookupVgs=bestVgs295K):
    ...
    for row in range(7):
      for col in range(7):
        ...
        rd.ncol = 7
        rd.nsamp = 1
        pedscan() # just to get the new Vgs value settled.
        # Create tuple for the index of just this pixel.  We could
        # actually pass thisPix = row,col  but then you can't do
        # an array.mean() on that one pixel because it returns a 
        # single value (Error: int object has no attribute mean).
