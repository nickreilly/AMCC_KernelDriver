...
from nsr_genfuncs import nap, set_array_vars, NC1_chars, nc1_sutr
...
def nc1_sutr(num_ramps, row_start=0, row_end=2048, number_garbage_frames=10):
    ...
    for i_sub, nrowskip in enumerate(nskips):
        ...
        for ramp in range(num_ramps):
            ...
            rd.lc = 'Ramp number:' + str(ramp) + ' of ' + str(num_ramps - 1)
...
rd.object = 'test_missionSUTR_full'
nc1_sutr(num_ramps, row_start=0, row_end=2048, number_garbage_frames=10)

rd.object = 'test_missionSUTR_sub'
nc1_sutr(num_ramps, row_start=nc1_chars.sub_start_row, row_end=nc1_chars.sub_end_row, number_garbage_frames=10)
