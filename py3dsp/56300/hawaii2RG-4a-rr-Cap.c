...
/* don't use gb_reset since it is easier to just set the voltage always on. */
...
void RegOutputMode()
{       /* This sets many things, but here it is used to turn on the extra
           row of reference pixels with varying capacitances (see Section 7.3
           in the HxRG manual).  Set REFROW=1 (bit #7)
         */
        /* 0011 0000 1111 0010 */
        int i;
...
/* Do Rising edge first on VClk, but later do falling edge first on HClk.
   This has to do with the fact that Col (horizontal) shift reg requires
   extra clocks to setup the register, while the row shift reg does not. */
void RowPhiReset()
{
	__asm("\n\
	move   #>RSTcount,R5 						\n\
	movep  #(RowOff+VClk),Y:RowPort	; rising edge, does nothing.	\n\
	rep  #RPhiUnderlap		; no reset on current row!	\n\
	nop								\n\
	movep  #(RowOff),Y:RowPort	; falling edge.	clocks ahead.	\n\
	rep  #RPhiUnderlap		; no reset yet because 		\n\
	nop				; don't want coincident clocks	\n\
	movep  #(RowOff+ResetEn),Y:RowPort ; reset current row.		\n\
	rep  R5 							\n\
	nop 								\n\
	");
}
...
void RowPhi()
{
	...
	nop								\n\
	movep  #(RowOff),Y:RowPort	; falling edge. clocks ahead. 	\n\
	rep  #RPhiUnderlap		; want same timing as RowPhiReset \n\
	nop								\n\
	movep  #(RowOff),Y:RowPort	; want same timing as RowPhiReset \n\
	rep  R5								\n\
	nop								\n\
	");
}
...
int ncols = ncolskip + (gb_ncols>>2);
    ...
    /* Unlike Col Shifter, Row Shifter doesn't need 2 extra clock for setup */
    ...
    if(gb_onepix)  /* reading just one pixel, over and over */
    {
	RowPhiReset();  /* the settle time is in this subroutine */
	ResetOff();  /* the settle time is in this subroutine */
	ColSync();   /* ColSync has an extra HClk in it for reg setup */
	ColPhiHi();  /* HClk Hi. Need extra clocking to setup H-reg. */
	...
	for(col = 0; col< ncolskip ; col++) /* burst furst  */
	{
    	...
    	for(row=0; row < gb_nrows ; row++) /* for each row in the image */
    ...
    else /* reading the array of pixels */
    {
	for(row=0; row < gb_nrows ; row++) /* for each row in the image */
	{
		/* Somewhere there is a little extra overhead of 800ns for 
		   just the very 1st RowPhiReset (rise of Vclk) to ADC 
		   (rise of ADC).  All the rest of either [RowPhiReset to ADC]
		   or [RowPhi to ADC] are the same timing.  
		   Could be overhead for "if (col==0) ColSettle();"?
		   Checked it, and no.  Also if true it should be for 
		   beginning of every row, not just the first row of 
		   the RstPed() frame.
		   Could be due to overhead of moving RSTcount into R5
		   for very 1st time used but then it is skipped by
		   compiler on subsequent rounds?
		   No, not that. 
		   Overhead for the burst clocking? no.
		   Is it "col = 0" in for loop? No. Set col=0 earlier in prog.
		   Comparing timing indicates the extra overhead occurs
		   at the ColSync() but I am still not sure how/why
		 */
		RowPhiReset();  /* the settle time is in this subroutine */
		ResetOff();  /* the settle time is in this subroutine */
		ColSync();   /* ColSync has an extra HClk in it for reg setup */
		...
		ColPhi(); /* might not be needed? generates LineCHK */
		ColPhiLo(); 
	}
    }
    readAdcs(); /* One more readAdcs to send out last converted pixels */
    RowPhi(); /* extra row clock to generate the FrameCHK */
...
int ncols = ncolskip + (gb_ncols>>2);
    ...
    if(gb_onepix)  /* reading just one pixel, over and over */
    {
	RowPhi();    /* the settle time is in this subroutine */
	ResetOff();  /* Reset is off already, but want same timing */
	ColSync();   /* ColSync has an extra HClk in it for reg setup */
	ColPhiHi();  /* HClk Hi. Need extra clocking to setup H-reg. */
...
void clockImage()
{
    ...
    RstPed(); /* first frame read has reset before each row */
    for (samps = 1 ; samps < gb_nsamps ; samps++) /* start at 1 not 0 */
    {
	    Sig();  /* do ped part of the Fowler image */
    }
    ReadEnOff();
...
void clockRampImage()
{
    ...
    end += gb_itime - 1; /* subtract 1ms for timing (always was long by 1ms)*/
    RstPed(); /* first frame read has reset before each row */
    ReadEnOff();
...
int temp = 0; /* 24 bit temp storage. */
    ...
    initSeq();
    initTTimer();
    /* Do a clockImage() or clockRampImage() here for easy simulation.
