...
/*
   Revision E
   Adds a second Reset Array pulse for each pixel.  Now there is both one
   before and one after the Self Reset Pulse.

   Revision D
   Make Self Reset and Reset Array pulses longer.
   Correct/Add the SUTR style data taking.  The integration time is NOT
   used to make any change in length of the period of sampling or bewteen
   samples in this Revision D.  The integration time is used just to put
   a delay for the opening and closing of the shutter which needs ~50msec.
 
   Revision C
   This clock also has the ability to output just the Ped frame 
   and NOT do shutter or Sig. 

   Revision B
   This revision of the clock does the first sample (ADC pulse) of 
   the CDS pair AFTER the reset.   
 
   For the integration time 
   There seems to be a bug that doesn't allow integrations longer than 64ms 
   (I think??).  Which may be related to the register that is used to store
   the number for the wait during the integration, i.e. need the 24-bit
   or 56-bit instead of 16-bit register.  This can only be fixed by writing
   directly in assembly that section of code instead of mixed C & assembly.
 
   Also, shutter is related to integration time ---
   Since we currently can't do longer than 64ms, that actually provides an
   easy way to make an adjustable shutter CLOSED time which is equal to the
   the shutter open integration time.  This allows for very short times 
   between pixels when you are using an electronic shutter and have short
   integration times.  And appropriately long open and close times when you
   are using a physical shutter.

   NOTE: because there is a SINGLE capacitor on chip that is 100fF, which is
   located between the row select and the source follower output FET, we
   have no choice but to do shutter per pixel operation.  There is the 
   exception of when very large capacitors are put onto the unbuffered lines
   for each row, but then you will still need a shutter per row at a minimum.
...
/*
 * This version of the clock will run on the 4 channel OCIW controller.
 *
 * 
 * 
*/
...
Clock3   EQU     Seq12 ; Pin 3  \n\
Clock4   EQU     Seq13 ; Pin 4  \n\
...
ResetArr EQU     Seq15 ; Pin 20 \n\
SelfRst  EQU     Seq11 ; Pin 21 \n\
VgsOn    EQU     Seq16 ; Pin 22 \n\
Clock23  EQU     Seq17 ; Pin 23 \n\
Clock24  EQU     Seq8  ; Pin 24 \n\
...
RowOff   EQU     Off+ResetRSB+ResetArr+VgsOn ; \n\
...
int gb_sutr = 0; /* 0=normal, 1=SUTR */
int gb_onepix = 0; /* 0=normal, 1= Single pixel reads */
int gb_pedonly = 0;  /* toggle PED or PEDSIG mode: 0=PEDSIG 1=PED */
...
void SelfReset()
{ 
	/* Really, reset doesn't need to clock through rows.
	   But, we still need have the reset applied for some time */
	__asm("\n\
	move    #>RSTcount,R5 \n\
    movep   #(RowOff+SelfRst),Y:RowPort ; SelfRst on, ResetArr off \n\
    rep     R5 \n\
    nop \n\
    rep     R5 \n\
    nop \n\
    ...
    rep     R5 \n\
    nop \n\
    movep   #(RowOff),Y:RowPort ; SelfRst off and ResetArr off \n\
    rep     R5 \n\
    nop \n\
	");
}
...
{ 
	...
	__asm("\n\
	move    #>RSTcount,R5 \n\
    movep   #(RowOff+SelfRst+D1RS),Y:RowPort ; SelfRst on, ResetArr off \n\
    rep     R5 \n\
    nop \n\
    rep     R5 \n\
    nop \n\
    ...
    rep     R5 \n\
    nop \n\
    movep   #(RowOff+D1RS),Y:RowPort ; SelfRst off and ResetArr off \n\
    rep     R5 \n\
    nop \n\
	");
}
...
{ 
    ...
    __asm("\n\
    move    #>RSTcount,R5 \n\
    movep   #(RowOff-ResetArr),Y:RowPort	; assert reset enable \n\
    rep		R5 \n\
    nop \n\
    rep     R5 \n\
    nop \n\
...
{ 
    ...
    __asm("\n\
    move    #>RSTcount,R5 \n\
    movep   #(RowOff-ResetArr+D1RS),Y:RowPort ; assert reset enable \n\
    rep		R5 \n\
    nop \n\
    rep     R5 \n\
    nop \n\
...
void ResetPixOff1()
{
    ...
    __asm("\n\
    move    #>RSTcount,R5 \n\
    movep   #(RowOff+D1RS),Y:RowPort	; remove reset enable \n\
    rep		R5 \n\
    nop \n\
        ");
}
...
int end,samps;
    ...
    ;
    end = timer() + gb_itime - 1; /* current time + integration*/
    shutterOpen();
    ...
    shutterClose(); 
    /* Need to keep the shutter closed about equal time as open (itime) */
    end = timer() + gb_itime; /* current time + integration*/
    while( !((end - timer())&0x800000) ) /* wait till itime is done */
    {
        checkForCommand();
        /* long integrations are trouble for temp control! */
...
int end,samps;
    ...
    SelfReset();
    ResetPixOn();
    readAdcs(); /* First read is during reset */
    ResetPixOff();
    readAdcs(); /* Second read is after reset, but shutter closed */

    shutterOpen();
    
    for (samps = 2 ; samps < gb_nsamps ; samps++)
    {
        readAdcs(); /* Read N-2 more times after shutter is open */
    }
    /* All the important stuff is done.  Now just waiting for slow shutter */
    end = timer(); /* what time is it now? */
    while (timer() == end) /*  wait for an edge. */
    ;
    end = timer() + gb_itime - 1; /* current time + integration*/
...
/* For this array, we want to read the row direction fast and column
 direction slowly, which is the reverse of most arrays.  It doesn't matter
 since row and column are identical.  We want to do this because we
 have the option of putting external capacitors on each row, which would
 allow integration for each row with only one reset per row. */
    start = timer();
    ...
    if(gb_onepix)
    {
        ColSync();
        Col1on();
        for(col=0 ; col < gb_ncolskip; col++) /* skipping cols. */
        {
            if (col==0)
            {
                /* col==0  so this is first col. */
                Col1Phi();
                Col1off();
            }
            else
        ...
        for(row=0 ; row < gb_nrowskip; row++) /* skipping rows. */
        {
            ...
            {
                RowSync();
                Row1on();  /* Row 1 pixels are separate from shift reg */
                Row1Phi(); /* clock to next pixel */
                Row1off(); /* turn off Row 1 Bit */
            }
            else
            {
                RowPhi(); /* clock to next pixel */
            }
        }
    ...
    else /* Reading multiple array pixels -- NOT single */
    {
        ColSync();
        Col1on();
        for(col=0 ; col < gb_ncolskip; col++) /* skipping cols. */
        ...
        /* Done skipping cols, now we are on the column of interest.
        ...
        for( ; col < ncols; col++) /* each col in image. */
        {
            ...
            /* Done skipping rows, now we are on the row of interest. */
...
void clockPed()
{
    ...
    gb_pedonly = 1; /* set parameter to Ped only */
    PedSig();  /* do ped */
    gb_pedonly = 0; /* set parameter back to PedSig */
    gb_pedtime = timer() - start ;
