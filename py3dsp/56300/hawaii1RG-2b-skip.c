...
/*
 ***** IMPORTANT! *** This clock is double clocked to read every other pixel!
...
RegUnderlap   EQU  @cvi(ClockRate*2000*NanoSec-8) \n\
");
...
int i;
	...
	{
		RowPhiReset();
		RowPhiReset();  /* double clock VCLK */

	}
	if (!gb_reset )
	{
		RowPhiNoReset(); 
		RowPhiNoReset();  /* double clock VCLK */
