...
RSTcount      EQU  @cvi(ClockRate*100*MicroSec) \n\
...
{ 
	...
	__asm("\n\
	move    #>RSTcount,R5 \n\
	movep   #(RowOff-GlbReset+PhiRCDS),Y:RowPort	; assert reset enable \n\
	rep		R5 \n\
	nop \n\
	move    #>RSTcount,R5 \n\
	movep   #(RowOff+PhiRCDS),Y:RowPort	; remove reset enable \n\
	rep		R5 \n\
	nop \n\
	");
}
...
{ 
	...
	__asm("\n\
	move    #>RSTcount,R5 \n\
	movep   #(RowOff-GlbReset+D1RS+PhiRCDS),Y:RowPort ; assert reset enable \n\
	rep		R5 \n\
	nop \n\
	move    #>RSTcount,R5 \n\
	movep   #(RowOff+D1RS+PhiRCDS),Y:RowPort	; remove reset enable \n\
	rep		R5 \n\
	nop \n\
	");
}
...
int end;
int samps;
    ResetArrayRow1();  /* Reset whole array */
    __asm("\n\
    movep #(RowOff+D1RS+PhiS1CDS+PhiRCDS),Y:RowPort ; Pedestal to CDS cap \n\
    rep #RPhiUnderlap \n\
    nop \n\
    rep #RPhiUnderlap \n\
    nop \n\
    movep #(RowOff+D1RS+PhiRCDS),Y:RowPort ; disconnect CDS S1 cap \n\
    nop \n\
    ");
    /* Now, do the integration calculation, then open shutter and wait */
    ...
    __asm("\n\
    movep #(RowOff+D1RS+PhiS2CDS+PhiRCDS),Y:RowPort ; Signal level to CDS cap \n\
    rep #RowSyncCount \n\
    nop \n\
    movep #(RowOff+D1RS),Y:RowPort ; disconnect CDS S2 cap \n\
    rep #20 \n\
    nop \n\
    movep #(RowOff+D1RS+PhiTCDS),Y:RowPort ; Signal level to CDS cap \n\
    rep #RPhiUnderlap \n\
    nop \n\
    ");
    for (samps = 0 ; samps < gb_nsamps ; samps++)
    {
	readAdcs(); /* Read the voltage, do A/D conversion */
    ...
    rep #20 \n\
    nop \n\
    movep #(RowOff+D1RS+PhiRCDS),Y:RowPort ; reset CDS C2 cap \n\
    rep #RowSyncCount \n\
    nop \n\
    ");
    shutterClose();
    /* Make sure shutter is closed for minimum time, so wait some time */
    end = timer() + 10; /* what time is it now? plus 10 ms */ 
    while( !((end - timer())&0x800000) ) /*  wait for some time. */
        ;
}
...
int end;
int samps;
    ResetArray();  /* Reset whole array, but really just current pixel needed */
    __asm("\n\
    movep #(RowOff+PhiS1CDS+PhiRCDS),Y:RowPort ; Pedestal level to CDS cap \n\
    rep #RPhiUnderlap \n\
    nop \n\
    rep #RPhiUnderlap \n\
    nop \n\
    movep #(RowOff+PhiRCDS),Y:RowPort ; disconnect CDS S1 cap \n\
    nop \n\
    ");
    /* Now, do the integration calculation, then open shutter and wait */
    end = timer(); /* what time is it now? */
    ...
    __asm("\n\
    movep #(RowOff+PhiS2CDS+PhiRCDS),Y:RowPort ; Signal level to CDS cap \n\
    rep #RowSyncCount \n\
    nop \n\
    movep #(RowOff),Y:RowPort ; disconnect CDS S2 cap \n\
    rep #20 \n\
    nop \n\
    movep #(RowOff+PhiTCDS),Y:RowPort ; Signal level to CDS cap \n\
    rep #RPhiUnderlap \n\
    nop \n\
    ");
    for (samps = 0 ; samps < gb_nsamps ; samps++)
    ...
    rep #20 \n\
    nop \n\
    movep #(RowOff+PhiRCDS),Y:RowPort ; Reset CDS C2 cap \n\
    rep #RowSyncCount \n\
    nop \n\
    ");
    shutterClose();
...
void PedSig()
{
/* There is no PedSig() in this clock because of the way 
Reset, Ped, Integrate & Sig must be done while still on the same pixel */
} 
...
int samps;
	...
	for(row=0 ; row < gb_nrows; row++) /* each row in image. */
	{
	    ...
	    {
		PedSigCDS(); /* Read same pixel ncol*nrow times */
		ColSettle(); /* just want to wait a little between reads */
	    }
	}
    }
