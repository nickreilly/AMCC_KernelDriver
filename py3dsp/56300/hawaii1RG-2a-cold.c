...
;; with modifications by Drew Moore, RIT.
  ...
  Changed some timing to run slower when the ROIC is colder than ~27K.
...
/*
 * This version of the HAWAII-1RG clock will run on the 4 channel OCIW controller.  
 *
...
RowSyncCount  EQU  @cvi(ClockRate*3.0*MicroSec) \n\
RPhiUnderlap  EQU  @cvi(ClockRate*3*MicroSec) \n\
RPhiRUnderlap EQU  @cvi(ClockRate*10*MicroSec) \n\
ColSyncCount  EQU  @cvi(ClockRate*3.0*MicroSec) \n\
CPhiUnderlap  EQU  @cvi(ClockRate*500*NanoSec-8) \n\
Clamp2Samp    EQU  @cvi(ClockRate*5000*NanoSec-8) \n\
RegUnderlap   EQU  @cvi(ClockRate*20.0*MicroSec) \n\
");
...
void readAdcs()
{
	...
	movep	y:Adc0,R0	; read last 2 pix into 2 handy registers \n\
	nop			; \n\
	movep	y:Adc1,R1	; \n\
	nop			; \n\
...
/*
  Setup programing for the HAWAII-1RG serial register (see manual).
*/
...
int row;
int col=0;
int ncolskip = gb_ncolskip>>1; /* gb_ncolskip divided by number of outputs  */
