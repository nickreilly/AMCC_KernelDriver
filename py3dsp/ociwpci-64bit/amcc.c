...
amcc_probe (struct pci_dev* pcidev, const struct pci_device_id* amcc_device_id)
{
    ...
    printk(KERN_DEBUG "amcc_probe: PT[%d] Size %8d  Phys %pK  Virt %pK\n",
        1, (int)(gb_dev.pt1_size), (void *)(gb_dev.pt1_phys_addr),
        (void *)(gb_dev.pt1_virt_addr));
...
amcc_read(struct file *file, char *buf, size_t count, loff_t *ppos)
{
    ...
    a2p.bytes_done = 0;
    a2p.copy_size = min((unsigned long)count, (unsigned long)a2p.size);
...
amcc_write(struct file *file, const char *buf, size_t count, loff_t *ppos)
{
    ...
    while (count > 0){
        p2a.bytes_done = 0;
        p2a.copy_size = min((unsigned long)count, (unsigned long)p2a.size);
