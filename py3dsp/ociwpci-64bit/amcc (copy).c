...
//    struct pci_dev *pcidev;
    ...
    uint32_t opreg_addr;             /* operation registers address BAR[0] */
    uint32_t opreg_len;              /* operation registers size */
    unsigned int opreg_type;             /* MEM or I/O operation registers */
    int ptnum;                  /* number of pass-thru regions */
    unsigned long pt1_phys_addr;       /* Pass-thru region 1 - BAR[1] */
    unsigned long pt1_size;            /* Pass-thru region 1 size */
    uintptr_t  pt1_virt_addr;       /* Virtual address after remapping */
    unsigned long pt2_phys_addr;       /* Pass-thru region 2 - BAR[2] */
    unsigned long pt2_size;            /* Pass-thru region 2 size */
    uintptr_t  pt2_virt_addr;       /* Virtual address after remapping */
    int hfmode;
};
...
static long lock = 0UL;       /* generic lock dword */
...
static irqreturn_t amcc_irq_handler (int, void *);
...
long amcc_physbufsize = 0; // size of physical memory we can use
//MODULE_PARM(ociw_physbufsize,"l"); // size of physical buffer in bytes
module_param(amcc_physbufsize, long, 0644);
...
{
    return ((uint32_t) inl (gb_dev.opreg_addr + reg));
}

static inline void
amcc_write_opreg (uint32_t value, long reg)
{
    outl (value, (uint16_t) (gb_dev.opreg_addr + reg));
}

static void hf_ints(void) // called from ISR at high priority
{
	writel(0x00010001,(uint32_t *)gb_dev.pt2_virt_addr); // enable HF flag ints
...
static void ef_ints(void) // called from READ at low priority
{
	...
	writel(0,(uint32_t *)gb_dev.pt2_virt_addr); // enable empty flag ints
...
{
	amcc_write_opreg ((uint32_t) ICSR_ADDON_ENABLE,  AMCC_OP_REG_ICSR);
}

static void ints_off(void)
{
	amcc_write_opreg ((uint32_t) 0 ,  AMCC_OP_REG_ICSR);
}
...
amcc_probe (struct pci_dev* pcidev, const struct pci_device_id* amcc_device_id)
{
    ...
    if (amcc_physbufsize) // physical buffer available?
    {

	    phy_buf = vmalloc(amcc_physbufsize); // alloc for pixels.
	    // check if it worked! complain if not!!
        if(!phy_buf) // null pointer? not a good sign.
        {
            // 
		    printk(KERN_WARNING "Could not allocate amcc_physbuf!!");
		    printk(KERN_WARNING "Recompile amcc.c with OCIW_DEBUG for debug printk's" );
            __unregister_chrdev (gb_dev.major, AMCC_BASE_MINOR, 1, AMCC_NAME);
            kfree(p2a.buf);
            kfree(a2p.buf);
		    return (-ENOMEM);
        ...
        else
        {
            ...
            phy_buf_end = phy_buf+(amcc_physbufsize/sizeof(short)); // DON'T write here!!
    ...
    printk (KERN_DEBUG "amcc_probe: PT[%d] Size %8x  Phys %08x  Virt %08lx\n",
        1, (gb_dev.pt1_size), (gb_dev.pt1_phys_addr),
        (u_long)(gb_dev.pt1_virt_addr));
    ...
    amcc_write_opreg ((uint32_t) 0, AMCC_OP_REG_RCR);

    amcc_write_opreg ((uint32_t) 0, AMCC_OP_REG_ICSR);
    amcc_write_opreg ((uint32_t) 0, AMCC_OP_REG_PTCR);

    gb_dev.irq = pcidev->irq;  
    if ( request_irq(gb_dev.irq, 
                 &amcc_irq_handler,IRQF_SHARED | 0,"amcc",(void*)&gb_dev)<0)
    ...
    printk (KERN_DEBUG "amcc_probe: ICSR Expected: 0x00000C0C  RCR Expected: 0x00000000\n");
    printk (KERN_DEBUG "amcc_probe: ICSR Measured: 0x%08x  RCR Measured: 0x%08x\n",
        amcc_read_opreg(AMCC_OP_REG_ICSR), amcc_read_opreg(AMCC_OP_REG_RCR));
    printk (KERN_DEBUG "amcc_probe: MBEF Expected: 0x80000000 PTCR Expected: 0x80808080\n");
    printk (KERN_DEBUG "amcc_probe: MBEF Measured: 0x%08x PTCR Measured: 0x%08x\n",
...
amcc_remove (struct pci_dev* pcidev)
{
    ...
    static int amcc_probe(struct pci_dev *pcidev, const struct pci_device_id *amcc_device_id) {
    extern struct amcc_priv gb_dev;
    int rc, j, err = 0;
    dev_t devno;
    struct device *device = NULL;
    ...
    // Initialize the gb_dev struct with information about the PCI resources
    gb_dev.pt1_size = pci_resource_len(pcidev, 1);
    gb_dev.pt1_phys_addr = pci_resource_start(pcidev, 1);
    gb_dev.pt1_virt_addr = ioremap(gb_dev.pt1_phys_addr, gb_dev.pt1_size);
    ...
    printk(KERN_DEBUG "amcc_probe: PT[%d] Size %8x  Phys %08x  Virt %08lx\n",
        1, gb_dev.pt1_size, gb_dev.pt1_phys_addr, (unsigned long)gb_dev.pt1_virt_addr);

    // Reset the board and set some defaults
    amcc_write_opreg(RCR_RESET_MBFLAGS, AMCC_OP_REG_RCR);
    amcc_write_opreg(RCR_RESET_ADDON, AMCC_OP_REG_RCR);
    ...
    amcc_write_opreg(0, AMCC_OP_REG_RCR);
    amcc_write_opreg(0, AMCC_OP_REG_ICSR);
    amcc_write_opreg(0, AMCC_OP_REG_PTCR);

    gb_dev.irq = pcidev->irq;

    // Request the IRQ and enable interrupts
    if (request_irq(gb_dev.irq, &amcc_irq_handler, IRQF_SHARED | 0, "amcc", (void *)&gb_dev) < 0)
        printk(KERN_WARNING "request irq failed\n");
    else {
        ef_ints();
        ints_on();
...
amcc_read(struct file *file, char *buf, size_t count, loff_t *ppos)
{
    ...
    ptr = (uint32_t *)(gb_dev.pt1_virt_addr);
    rbuf = (u16 *)a2p.buf;
    nwords = 0;

    a2p.bytes_done = 0;
    a2p.copy_size = min((size_t)count, (size_t)a2p.size);
...
/*  If the FIFO is not ready, return immediately.               */
    for (j=0; j<a2p.copy_size/2; j++) {
        if (!(amcc_read_opreg (AMCC_OP_REG_IMB) & IMB_RX_RDY))
            break;
        rbuf[j] = (u16) readl ((uint32_t *)(&ptr[j]));
        a2p.bytes_done +=2;
    }
#endif
        ...
        while (a2p.bytes_done < a2p.copy_size  && retrys < AMCC_MAX_RETRYS) {
            ...
            if (r & IMB_RX_RDY){
                if (a2p.copy_size-a2p.bytes_done > 255 && (r & IMB_RX_PAE) ){
                    for (k=0; k<127; k++){
                        rbuf[j] = (u16) readl ((uint32_t *)(&ptr[k]));
                        j++;
                        a2p.bytes_done += 2;
                    }
                }
                else {
                    rbuf[j] = (u16) readl ((uint32_t *)(&ptr[0]));
                    a2p.bytes_done +=2;
                    j++;
                }
                retrys=0;
...
amcc_write(struct file *file, const char *buf, size_t count, loff_t *ppos)
{
    ...
    while (count > 0){
        p2a.bytes_done = 0;
        p2a.copy_size = min((size_t)count, (size_t)a2p.size);
        ...
        for (j=0; j<p2a.copy_size/2; j++){
            ...
            writel ( (uint32_t)wbuf[j], (uint32_t *)(&ptr[j]) );
            udelay(5);
            p2a.bytes_done += 2;
        }
    ...
    unsigned int cmd, unsigned long arg)
{
    ...
    case AMCC_RESET_FIFO:       /* reset the PCI card external FIFO */
        for (j=0; j<AMCC_FIFO_SIZE; j++) {
            if (!(amcc_read_opreg (AMCC_OP_REG_IMB) & IMB_RX_RDY))
                break;
            (void)readl((uint32_t *)(gb_dev.pt1_virt_addr));
            count +=2;
        }
        pixqueue_head = phy_buf; // read owns this pointer
        pixqueue_tail = phy_buf; // irq owns this pointer
...
// stick them into the queue.
{
    ...
    {
		// read the next pixel
        *pixqueue_tail = (u16) readl ((uint32_t *)(gb_dev.pt1_virt_addr));
...
static irqreturn_t
amcc_irq_handler (int irq, void *dev)
{
	uint32_t icsr = 0;
...
amcc_init (void)
{
    ...
    printk (KERN_INFO "amcc_init: AMCC S5920 Rev 3.01a for 5.19 kernel \n");
