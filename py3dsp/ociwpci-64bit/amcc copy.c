...
//MODULE_PARM(amcc_physbufsize,"l"); // size of physical buffer in bytes
module_param(amcc_physbufsize, long, 0644);
//__MODULE_PARM_TYPE(amcc_physbufsize,"l"); // size of physical buffer in bytes
...
amcc_probe (struct pci_dev* pcidev, const struct pci_device_id* amcc_device_id)
{
    ...
    // struct device *temp_device = NULL;
    ...
    rc = __register_chrdev(AMCC_MAJOR, AMCC_BASE_MINOR, 1, AMCC_NAME, &amcc_fops);
    ...
    printk(KERN_DEBUG "amcc_probe: PT[%d] Size %8zd Phys %08lx Virt %08lx\n", 
        1, gb_dev.pt1_size, (unsigned long) gb_dev.pt1_phys_addr, 
        (unsigned long) gb_dev.pt1_virt_addr);
    ...
    if ( request_irq(gb_dev.irq, 
                 &amcc_irq_handler,IRQF_SHARED | IRQF_NO_THREAD,"amcc",(void*)&gb_dev)<0)
...
amcc_init (void)
{
    ...
    printk (KERN_INFO "amcc_init: AMCC S5920 Rev 3.01 for 5.15 kernel \n");
